...
fn parse_input(input: &str) -> Vec<HashMap<String, i32>> {
    ...
    // filter the ones that have either blue > 14, red > 12, or green > 13
    let filtered_input: Vec<HashMap<String, i32>> = parsed_input
        .into_iter()
        .filter(|game| game["blue"] <= 14 && game["red"] <= 12 && game["green"] <= 13)
        .collect();
